# This reads in DESeq2 differential OTU abundance results for two sets of results:
...
DESeq2ResultsRemn <- load("~/Box Sync/R_code/Gradient_SequenceAndEnviron_Analysis/ScriptsForGitHubArchiving/autoGeneratedScriptOutput/GradientCoordinated_DESEQ2_results_remnantSites.r")
...
# Load the data.
DESeq2ResultsAllSites <- load("~/Box Sync/R_code/Gradient_SequenceAndEnviron_Analysis/ScriptsForGitHubArchiving/autoGeneratedScriptOutput/GradientCoordinated_DESEQ2_results_allSamples.r")
...
# mtext(text = c("(Sens. to disturb.)", "(Not sens. to disturb.)"), side = 2,
#       at = c(0.0, 0.75), las = 3, line = 0.3, cex = 1.5)

mtext(text = c("All samples"), side = 2,
...
mtext(text = c("Remnant samples only"), side = 3,
...
#abline(h = c(0.25, 0.75), lwd = c(7,1))
